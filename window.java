...
public class window {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            DatabaseManager db = new DatabaseManager(); // Initialize the DB
    ...
    client.setMessageListener(msg -> {
        ...
        if (parts.length == 2) {
            ...
            db.saveMessage(sender, sender + ": " + content); 
        } else {
    ...
    chatPanel.addMessage("SERVER", "Could not connect to server.");
            ...
            InputPanel inputPanel = new InputPanel(chatPanel, client);
            ProfilePanel profilePanel = new ProfilePanel(chatPanel);
            inputPanel.setDatabaseManager(db); // Add this (we will add the method next)
            profilePanel.setDatabaseManager(db);
