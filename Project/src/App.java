...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            // Set up TCP client after GUI is created
            initializeTCPClient();
        });
    }
    
    public static void initializeTCPClient() {
        try {
            client = new TCPClient(username);
            
            // FIX: Update the inputPanel with the live client connection
            if (inputPanel != null) {
                inputPanel.setClient(client);
            }
            
            // Listener to handle messages coming FROM the server
            client.setMessageListener((channel, msg) -> {
                ...
                if (parts.length == 2) {
                    ...
                    // This handles "SERVER: Welcome..." messages
                    chatPanel.addMessage("SERVER", msg);
                }
            });

        } catch (Exception e) {
            e.printStackTrace();
            chatPanel.addMessage("SERVER", "Could not connect to server. Is it running?");
        }
    }
    
    public static void createGUI(){
        frame = new JFrame("CS Messenger - " + username);
    ...
    public static void createChatPanel(){
        ...
        inputPanel = new InputPanel(chatPanel, client, Utilities.BACKGROUND);
        profilePanel = new ProfilePanel(chatPanel, Utilities.BACKGROUND);

        // Inject database manager into panels
